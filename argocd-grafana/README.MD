...
## 2nd Method install argocd with values.yaml
```bash
server:
  extraArgs:
    - --insecure
...
# 2.2 Install Prometheus stack with Grafana configuration
```bash
...
# 2.3 Install Prometheus Blackbox Exporter
```bash
helm install stable prometheus-community/prometheus-blackbox-exporter --version 11.8.0 -n prometheus
```

# 2.4 Apply the ReferenceGrant (after namespace exists)
```bash
kubectl apply -f prometheus-referencegrant.yaml
```

# 2.5 Apply the Grafana HTTPRoute (you already have this in http.yaml)
```bash
kubectl apply -f http.yaml
```

# 2.6 Get Grafana admin password
```bash
